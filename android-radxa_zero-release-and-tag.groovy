pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
            	git credentialsId: 'tesla-android-jenkins', url: 'https://github.com/tesla-android/android-amlogic.git', branch: "main"
            }
        }
        ...
        stage('Capture artifacts') {
            steps {
                script {
                    ARTIFACT_NAME = 'TeslaAndroid-' + RELEASE_VERSION + '-RELEASE-radxa_zero'
                }
                dir("out") {
                    sh('mv tesla_android_radxa_zero-ota-' + getBuildNumber() + '.zip ' + ARTIFACT_NAME + '-OTA.zip')
